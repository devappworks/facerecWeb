...
class TestRecognitionService:
    ...
    def recognize_face_comparison(image_bytes, domain: str, image_id: Optional[str] = None,
        ...
        logger.info(f"\n\n{'='*80}")
        logger.info(f"Starting A/B comparison for image: {image_id}")
        logger.info(f"Domain: {domain}")
        logger.info(f"Ground truth: {ground_truth}")
        logger.info(f"Image size: {len(image_bytes) if isinstance(image_bytes, bytes) else image_bytes.getbuffer().nbytes} bytes")
        logger.info(f"{'='*80}")

        # Run Pipeline A (current system - VGG-Face)
        logger.info(f"\n{'-'*80}")
        logger.info(f"PIPELINE A: VGG-Face (Current Production System)")
        logger.info(f"{'-'*80}")
        ...
        logger.info(f"  Status: {result_a.get('status')}")
        logger.info(f"  Faces detected: {result_a.get('total_faces_detected', 'N/A')}")
        logger.info(f"  Recognized: {result_a.get('person', 'None')}")
        logger.info(f"  Confidence: {result_a.get('confidence', 0):.2f}%")

        # Run Pipeline B (ArcFace system - state-of-the-art)
        logger.info(f"\n{'-'*80}")
        logger.info(f"PIPELINE B: ArcFace (State-of-the-art - 2019)")
        logger.info(f"{'-'*80}")
        ...
        logger.info(f"  Status: {result_b.get('status')}")
        logger.info(f"  Faces detected: {result_b.get('total_faces_detected', 'N/A')}")
        logger.info(f"  Recognized: {result_b.get('person', 'None')}")
        logger.info(f"  Confidence: {result_b.get('confidence', 0):.2f}%")
        ...
        logger.info(f"Both succeeded: {metrics.get('both_succeeded', False)}")
        logger.info(f"Results match: {metrics.get('results_match', False)}")

        # Handle None values for confidence and time differences
        conf_diff = metrics.get('confidence_difference')
        logger.info(f"Confidence diff: {conf_diff:.2f}%" if conf_diff is not None else "Confidence diff: N/A")

        time_diff = metrics.get('processing_time_difference')
        logger.info(f"Time diff: {time_diff:.2f}s" if time_diff is not None else "Time diff: N/A")

        logger.info(f"Faster pipeline: {metrics.get('faster_pipeline', 'N/A')}")
        ...
        logger.info(f"\n{'='*80}")
        logger.info(f"A/B COMPARISON COMPLETE")
        logger.info(f"Total time: {total_time:.2f}s")
        logger.info(f"Pipeline A: {time_a:.2f}s | Pipeline B: {time_b:.2f}s")
        logger.info(f"Speedup: {((time_a - time_b) / time_a * 100):.1f}% {'faster' if time_b < time_a else 'slower'}")
